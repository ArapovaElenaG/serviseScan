...
// import '../../login/FormsLogin/FormsLogin.scss'; - КАК ПОДЦЕПЛЯЕТСЯ ЭТОТ СТИЛЬ К ЭТОМУ КОМПОНЕНТУ???????
import infoCircle from '../../login/FormsLogin/Input/images/info-circle.svg';
import Input from '../../login/FormsLogin/Input/Input';
import Button from '../../login/FormsLogin/Button/Button';
...
function FormSearch () {
    ...
    React.useEffect(() => {
        // let lastForm = JSON.parse(localStorage.getItem('lastForm'));
        let lastINN = JSON.parse(localStorage.getItem('lastINN'));
        // let lastTonality = JSON.parse(localStorage.getItem('lastTonality'));
        // let lastLimit = JSON.parse(localStorage.getItem('lastLimit'));
        // let lastBeginDate = JSON.parse(localStorage.getItem('lastBeginDate'));
        // let lastEndDate = JSON.parse(localStorage.getItem('lastEndDate'));

        // if (lastForm) {setForm(lastForm)}
        if (lastINN) {setForm({...form, inn: lastINN})}
        // if (lastTonality) {setForm({...form, tonality: lastTonality})}
        // if (lastLimit) {setForm({...form, limit: lastLimit})}
        // if (lastBeginDate) {setForm({...form, period: {...form.state, beginDate: lastBeginDate}})}
        // if (lastEndDate) {setForm({...form, period: {...form.state, endDate: lastEndDate}})}
    }, [])

    // после изменения стейа form записываем в localStorage обновленный стейт
    React.useEffect(() => {
        // localStorage.setItem('lastForm', JSON.stringify(form));
        localStorage.setItem('lastINN', JSON.stringify(form.inn));
        // localStorage.setItem('lastTonality', JSON.stringify(form.tonality));
        // localStorage.setItem('lastLimit', JSON.stringify(form.limit));
        // localStorage.setItem('lastBeginDate', JSON.stringify(form.period.beginDate));
        // localStorage.setItem('lastEndDate', JSON.stringify(form.period.endDate));
    }, [form])
    ...
    const validate = (id, value) => {
        ...
        // для формата dd.mm.yyyy  dd/mm/yyyy  dd-mm-yyyy
        // const regExpDate = /^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/;
    ...
    // функция проверки дат. Если даты в будующем времени или конечная дата раньше начальной - записываем в стейт ошибку
    const handleDate = (name, value) => {
        ...
        if (!value) {
            ...
            // if (today < formatValue) {setForm({...form, period: {...form.period, invalid: 'Даты не должны быть в будущем времени'}})}
    ...
    const sendRequest = (event) => {
        ...
        // // для данных запроса собираем в массив histogramTypes значения полноты и рискфакторов
        // // не используется. По умолчанию в массив сразу попадают оба значения
        // const histogramTypes = [];
        // for (let key in form.histogramTypes) {
        //     if (form.histogramTypes[key]) {histogramTypes.push(key)}
        // };
        ...
        // // обычный запрос axios 
        // // url для запроса гистограммы
        // const urlSearch = `https://gateway.scan-interfax.ru/api/v1/objectsearch/histograms`;
        ...
        // запрос чрез создание экземпляра
        // // общая конфигурация для всех запросов
        // const configAxios = {
        //     headers: {
        //         "Authorization": `Bearer ${token.accessToken}`,
        //     }
